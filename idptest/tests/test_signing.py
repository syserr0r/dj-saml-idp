...
def test_loading_private_key():
    filename = os.path.join(PROJECT_ROOT, 'keys/sample/sample-private-key.pem')
    config = {smd.PRIVATE_KEY_FILENAME: filename}

    assert type(filename) is six.text_type
...
def test_loading_certificate_from_file():
    filename = os.path.join(PROJECT_ROOT, 'keys/sample/sample-certificate.pem')
